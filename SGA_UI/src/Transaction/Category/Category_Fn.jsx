...
function Category_Fn() {
    ...
    const GetCateDetail = async (event) => {
        try {
            let iseq = 1;
            if (categoryMainlList.length > 0){
                iseq = categoryMainlList[categoryMainlList.length - 1].sgap_seq + 1
            }
            const response = await axios.get(
                `${import.meta.env.VITE_API}/transaction/getTargetDetail_FromMaster?P_SGA_NO=${STC_Header.P_SGA_NO}&P_SEQ=${iseq}&P_TYPE_CODE=${event.value}&P_SUBJECT=${txtSubject.replace(/&/g, 'rrandrr').replace(/'/g, 'rrzzrr')}`,
                {
                    headers: {
